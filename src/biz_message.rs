...
impl Default for BizRouteInfo {
  /// 模拟C++的memset(this,0,sizeof(tagBizRouteInfo))
  fn default() -> Self {
    Self {
...
pub(crate) trait IBizMessage: IKnown {
  ...
  unsafe fn set_target_info(&self, target_info: BizRouteInfo);
  unsafe fn get_target_info(&self, target_info: &mut BizRouteInfo);

  unsafe fn set_send_info(&self, send_info: BizRouteInfo);
  unsafe fn get_send_info(&self, send_info: &mut BizRouteInfo);
...
pub(crate) struct F2PackerVTable {
  ...
  pub set_target_info: unsafe extern "C" fn(this: *mut c_void, target_info: BizRouteInfo),
  pub get_target_info: unsafe extern "C" fn(this: *mut c_void, target_info: *mut BizRouteInfo),
  pub set_send_info: unsafe extern "C" fn(this: *mut c_void, send_info: BizRouteInfo),
  pub get_send_info: unsafe extern "C" fn(this: *mut c_void, send_info: *mut BizRouteInfo),
...
#[repr(C)]
pub(crate) struct IBizMessageRust {
...
impl BizMessage {
  ...
  /// 设置目的地路由
  #[napi]
  pub fn set_target_info(&self, js_target_info: JSBizRouteInfo) -> Result<()> {
    let mut target_info = BizRouteInfo::default();
  ...
  /// 获取目的地路由
  #[napi]
  pub fn get_target_info(&self) -> Result<JSBizRouteInfo> {
    let mut target_info = BizRouteInfo::default();
  ...
  /// 设置发送者路由
  #[napi]
  pub fn set_send_info(&self, js_send_info: JSBizRouteInfo) -> Result<()> {
    let mut send_info = BizRouteInfo::default();
  ...
  /// 获取发送者路由
  #[napi]
  pub fn get_send_info(&self) -> Result<JSBizRouteInfo> {
    let mut send_info = BizRouteInfo::default();
    unsafe {
      self.check_ptr()?.get_send_info(&mut send_info);
    }
    let js_send_info = JSBizRouteInfo {
  ...
  pub fn get_error_info(&self) -> Result<String> {
    ...
    let error_info = unsafe { std::ffi::CStr::from_ptr(c_error_info).to_string_lossy().into_owned() };
    Ok(error_info)
  }
  /// 设置返回码
  #[napi]
